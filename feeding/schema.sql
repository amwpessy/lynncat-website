...
-- 喂猫员修改某个订单的内容和预约日期（p_photos 为 null 时保留原照片，传数组则整体替换）
...
begin
  ...
  update bookings set
    d = p_d, customer_name = p_name, customer_phone = p_phone,
...
-- 喂猫员按客户现有预约整体调整日期：已点亮的日期保留，点暗的相关订单删除，新点亮的日期新增订单。
...
begin
  ...
  update bookings set
    d = v_target,
    customer_name = coalesce(p_name, v_booking.customer_name),
    customer_phone = coalesce(p_phone, v_booking.customer_phone),
    address = coalesce(p_address, v_booking.address),
    pet_info = coalesce(p_pet, v_booking.pet_info),
    notes = coalesce(p_notes, v_booking.notes),
    photo_urls = v_photos
  where id = p_id;

  -- 只删除同一客户的相关订单，避免客户端传入无关订单 ID 时越权影响其他客户。
  delete from bookings
   where id = any(coalesce(p_related_ids, '{}'))
     and id <> p_id
     and customer_phone = v_booking.customer_phone
...
-- 喂猫员在修改日期时查看某个客户全部有效订单，用于避免重复选日期。
